...
import { fmtDateShort } from "@/_shared/utils";
      ...
      <div className="flex items-center gap-3 px-4 py-3 sm:gap-4 sm:px-8">
        ...
        {!showSearch && (
          <div className="hidden h-11 min-w-0 shrink-0 items-center gap-1 rounded-full border border-line bg-gradient-to-r from-white via-white to-[#f8f2e3] pl-1.5 pr-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.7),0_1px_3px_rgba(30,41,59,0.06)] lg:flex">
            {/* Which day this strip is showing. "Live" ONLY when today's rate
                has been published; otherwise the date it is actually from, so a
                yesterday figure is never read as today's. */}
            {bullion && bullion.isToday === false ? (
              <span
                className="flex shrink-0 items-center gap-1.5 rounded-full bg-canvas px-2.5 py-1.5 text-[10px] font-extrabold uppercase tracking-[0.12em] text-muted"
                title="Today's rate has not been published yet — this is the last published rate."
              >
                <span className="h-1.5 w-1.5 rounded-full bg-muted" />
                {fmtDateShort(bullion.effective_date)}
              </span>
            ) : (
